...
export default function HabitsPage() {
  ...
  const handleAiAsk = async (e: React.FormEvent) => {
    ...
    const context = habits.length > 0 
      ? `${t('habits.habitsContext')}${habitTitles}. Mi pregunta es: ${aiQuestion}`
      : `${t('habits.noHabitsContext')} Mi pregunta es: ${aiQuestion}`;

    const systemPrompt = "Eres un Asistente de Hábitos experto en productividad y formación de rutinas. Responde concisamente al usuario basándote en el contexto que te proporciona, y ofrece 2 consejos prácticos.";
    ...
    try {
      ...
      if (!res.ok) throw new Error(t('habits.aiAssistantError'));

      const data = await res.json();
      setAiAnswer(data.response);
    } catch (err) {
      setAiError(t('habits.aiAssistantError'));
      console.error(err);
    } finally {
      setAiLoading(false);
    }
  ...
  return (
    <main className="py-4">
      ...
      {/* 🆕 ASISTENTE DE HÁBITOS (AI) */}
      <section className="mb-8 bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
        <h2 className="text-lg font-semibold mb-3">{t('habits.aiAssistantTitle')}</h2>
        <p className="text-sm text-gray-600 mb-4">{t('habits.aiAssistantPrompt')}</p>
        ...
        <form onSubmit={handleAiAsk} className="flex flex-col sm:flex-row gap-3">
          <input
            ...
            placeholder={t('habits.aiAssistantPlaceholder')}
          ...
          <button
            type="submit"
            disabled={aiLoading}
            className="px-5 py-2 rounded-lg bg-indigo-600 text-white font-medium shadow hover:bg-indigo-700 transition disabled:bg-gray-400 disabled:cursor-not-allowed"
          >
            {aiLoading ? t('habits.aiAssistantLoading') : t('habits.aiAssistantButton')}
          </button>
        </form>
